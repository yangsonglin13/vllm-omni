...
pool. Do not use the Mooncake-style CPU pool for the Ascend fast path.
...
```bash
# Example: AR uses NPU 0..7 and DiT uses NPU 8..11.
# Replace the destination IPs with the values reported by hccn_tool -i 8..11 -ip -g.
...
`memory_pool_device: "cpu"` is not recommended for
...
`MooncakeTransferEngineConnector`, but Yuanrong TE on Ascend should use `"npu"`.

## Heterogeneous TP Example

For AR 8-way TP sending KV cache to DiT 4-way TP, set the stage TP topology in
...
In this topology each DiT rank receives two AR rank shards:

```text
DiT rank 0 <- AR ranks 0,1
DiT rank 1 <- AR ranks 2,3
DiT rank 2 <- AR ranks 4,5
DiT rank 3 <- AR ranks 6,7
```
...
Successfully received KV cache ... across 2 key(s)
