...
_AUTO_HOST_VALUES = {"", "auto", "*", "0.0.0.0", "::"}
_AUTO_DEVICE_VALUES = {"", "auto"}
...
def _resolve_device_name(configured_device: Any, protocol: str) -> str:
    device_name = str(_expand_env_value(configured_device or "auto")).strip()
    if device_name.lower() not in _AUTO_DEVICE_VALUES:
        return device_name
    if protocol.lower() in {"ascend", "hccl", "npu"}:
        return f"npu:{_resolve_local_rank()}"
    return ""
...
class YuanrongTransferEngineConnector(OmniConnectorBase):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.config = {key: _expand_env_value(value) for key, value in config.items()}
        config = self.config

        raw_host_config = config.get("host")
        host_config = "auto" if raw_host_config is None else str(raw_host_config).strip()
        self.host = self._get_local_ip() if host_config.lower() in _AUTO_HOST_VALUES else host_config
        ...
        self.sender_zmq_port = (
            None
            if sender_zmq_port is None or str(sender_zmq_port).strip().lower() in {"", "auto"}
            else int(sender_zmq_port)
        )
    ...
    def _resolve_port(configured_port: Any, host: str) -> int:
        configured_port = _expand_env_value(configured_port)
    ...
    def get(
        ...
        if not metadata:
            ...
            and metadata.get("source_rpc_port")
            and metadata.get("source_addrs")
        ):
